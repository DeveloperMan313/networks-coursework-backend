...
class Port_dtl(Port_phy):
    ...
    def __change_state(self):
        ...
        if (
            self.__state in (PS_dtl.INACTIVE, PS_dtl.STANDBY)
        ) and self.__current_request is not None:
            fail_res = MsgRes(self.__current_request, False)
            self.__put_to_receive_buffer(fail_res)
            self.__current_str_chunks.clear()
            self.__current_request = None

        match self.__state:
            case PS_dtl.INACTIVE:
                ...
                if head:
                    ...
                    match head:
                        ...
                        case PFrameH.DATASTART:
                            self.__current_str_chunks.clear()
                            self.__send_1chunk_frame(PFrameH.ACK)
                            self.__set_state(PS_dtl.RX_DATA_AWAIT_HEAD_OR_END)
                    return
                if not self.__send_buffer.empty():
                    req = self.__get_from_send_buffer()
                    if not self.__frame_head_must_be_of(
                    ...
                    match req:
                        case PFrameH.DOWNLINK:
                            ...
                            self.__set_state(PS_dtl.TX_DOWNLINK_AWAIT_ACK)
                        case PFrameH.LINKACTIVE:
                            self.__send_1chunk_frame(PFrameH.LINKACTIVE)
                            self.__set_state(PS_dtl.TX_LINKACTIVE_AWAIT_ACK)
                    return
                if not self.__send_str_buffer.empty():
            ...
            case PS_dtl.TX_DATA_SEND_HEAD_OR_END:
                if not self.__current_str_chunks:
                    ...
                    self.__set_state(PS_dtl.TX_DATAEND_AWAIT_ACK)
                    return
            ...
            case PS_dtl.RX_DATA_AWAIT_HEAD_OR_END:
                ...
                match head:
                    case PFrameH.DATA:
                        self.__send_1chunk_frame(PFrameH.ACK)
                        self.__set_state(PS_dtl.RX_DATA_AWAIT_DATA)
